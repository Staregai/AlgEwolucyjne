...
from functions.benchmark import benchmark_functions, benchmark_bounds, benchmark_known_min
...
class TestCMAESPerformance(unittest.TestCase):
    ...
    def run_optimizer(self, func, center_strategy, seed, dim):
        ...
        # tutaj do dodania mnożnik !!!! ważne  *dim lub math.log(dim)
        ...
        if curve is not None:
            curve_filename = f"{CONVERGENCE_DIR}/{func.__name__}_{type(center_strategy).__name__}_dim{dim}_seed{seed}.csv"
    ...
    def test_wilcoxon_all(self):
        if not self.results:
            if os.path.exists(RESULTS_PKL):
                ...
                self.skipTest(
                    "Brak wyników, najpierw uruchom test_performance_and_collect."
                )
