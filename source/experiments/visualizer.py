...
def plot_convergence_curves(curves_dir="results", func_name="rastrigin", strategies=None):
    ...
    for strategy in strategies:
        all_curves = []
        files = glob.glob(f"{curves_dir}/convergence/{func_name}_{strategy}_seed*.csv")
        for file in files:
            df = pd.read_csv(file)
