...
class cma_es:
    ...
    def optimize(self, func, max_iter=1000):
        for _ in range(max_iter):
            ...
            center = self.compute_new_center(best_pop)
            delta = center - self.m
            self.m = center
    ...
    def compute_new_center(self, best_pop):
        if isinstance(self.center_strategy, mn.WeightedMeanCenterStrategy):
            weights = np.linspace(len(best_pop), 1, len(best_pop))
            center = self.center_strategy.compute_center(weights, best_pop)
        elif isinstance(self.center_strategy, mn.TrimmedMeanCenterStrategy):
            center = self.center_strategy.compute_center(best_pop)
        else:
            center = self.center_strategy.compute_center(best_pop)
        return center
